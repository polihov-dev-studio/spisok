...
function updateCategoryRelatedSelects() {
  fillSelect(
    $('categoryFilter'),
    [{ value: 'all', label: 'Все' }, ...state.data.categories.map(cat => ({ value: cat.id, label: cat.name }))]
  );
  fillSelect(
    $('subCategoryParentSelect'),
    state.data.categories.map(cat => ({ value: cat.id, label: cat.name }))
  );
  fillSelect(
    $('itemCategory'),
    state.data.categories.map(cat => ({ value: cat.id, label: cat.name }))
  ...
  if (state.data.categories.length) {
    if (!$('subCategoryParentSelect').value) $('subCategoryParentSelect').value = state.data.categories[0].id;
    if (!$('itemCategory').value) $('itemCategory').value = state.data.categories[0].id;
  }
...
function syncModalSubcategories() {
  ...
  const category = state.data.categories.find(x => x.id === categoryId);
...
function addCategory() {
  ...
  if (!name) return;
  state.data.categories.push({ id: uid(), name, subcategories: [] });
  input.value = '';
  saveData();
  updateCategoryRelatedSelects();
  render();
...
function addSubcategory() {
  ...
  const category = state.data.categories.find(x => x.id === parentId);
  if (!category) return;
...
function openItemModal(item = null) {
  ...
  if (item) {
    ...
    $('itemCategory').value = item.categoryId || state.data.categories[0]?.id || '';
    syncModalSubcategories();
    ...
    $('itemPrice').value = '';
    $('itemQty').value = 1;
    $('itemCategory').value = state.data.categories[0]?.id || '';
    syncModalSubcategories();
    $('itemNote').value = '';
...
function saveItemFromModal() {
  ...
  if (state.editingId) {
    const old = state.data.items.find(i => i.id === state.editingId);
    prepared.createdAt = old?.createdAt || prepared.createdAt;
    state.data.items = state.data.items.map(item => item.id === state.editingId ? prepared : item);
  } else {
    state.data.items.unshift(prepared);
  }
...
function renderStats() {
  const active = state.data.items.filter(item => !item.bought);
  const activeSum = active.reduce((sum, item) => sum + item.price * item.qty, 0);
  const boughtSum = state.data.items.filter(item => item.bought).reduce((sum, item) => sum + item.price * item.qty, 0);
  const favorites = state.data.items.filter(item => item.favorite).length;
...
function renderSections() {
  ...
  const sectionsHtml = state.data.categories.map(category => {
...
function renderItemCard(item) {
  const category = state.data.categories.find(x => x.id === item.categoryId);
